...
# --- MERGED MAIN TICKER ANALYSIS FUNCTION ---
def analyze_single_ticker(
    ...
    try:
        ...
        # Price, Volume & Returns Calculation
        ...
        # Highs & Moving Averages
        df['High_20_Prev'] = df['High'].shift(1).rolling(20).max()
        df['Max_250_High_1d_Ago'] = df['High'].shift(1).rolling(250, min_periods=20).max()
        df['EMA_20'] = df['Close'].ewm(span=20, adjust=False).mean()
        df['EMA_50'] = df['Close'].ewm(span=50, adjust=False).mean()
        df['EMA_200'] = df['Close'].ewm(span=200, adjust=False).mean()

        # Roadmap Condition (50 EMA Touch / Cross Check)
        ...
        df['Cond_Roadmap'] = crossed_roadmap | touching_roadmap
        is_roadmap = bool(df['Cond_Roadmap'].iloc[-1]) if not df['Cond_Roadmap'].empty else False

        # Technical Indicators via 'ta' Library & Custom Math
        ...
        if is_signal and pd.notna(last_close_val):
            ...
            atr_v = float(last_row['ATR']) if pd.notna(last_row['ATR']) else (entry * 0.02)
            
            # SL and Target Logic with ATR Safeguard
            low_5d = float(last_row['Low_5d']) if pd.notna(last_row['Low_5d']) else entry * 0.95
            sl = max(low_5d - (atr_v * 0.2), entry * 0.95)
            ...
            avg_vol = float(last_row['Vol_SMA20'])
            vol_spike = curr_vol / avg_vol if avg_vol > 0 else 0
            buying_surge_pct = ((curr_vol - avg_vol) / (avg_vol + 1e-10)) * 100
            accum_ratio = (
                float(last_row['Accum_Ratio_10d'])
                if pd.notna(last_row['Accum_Ratio_10d'])
                else 1.0
            )
            ...
            close_pos = (
                ((entry - day_low) / day_range * 100) if day_range > 0 else 50.0
            )

            # Ranks and Execution Conditions
            if close_pos >= 90.0 and buying_surge_pct >= 200.0:
                exec_rank = '🥇 Rank 1 (Top Winner)'
                entry_window = f"₹{round(entry * 0.995, 2)} - ₹{round(entry * 1.005, 2)}"
                exec_condition = f'⚡ Instant Entry (Hold above ₹{round(entry, 2)})'
                ...
                entry_window = f"₹{round(entry * 0.99, 2)} - ₹{round(entry * 1.01, 2)}"
                exec_condition = f'⏳ Wait for Pullback near EMA 20'
            ...
            rsi_val = (
                float(last_row['RSI']) if pd.notna(last_row['RSI']) else 50.0
            )
            total_score = round(
                min(
                    rsi_val
            ...
            return [{
                ...
                'Roadmap Zone': '⭐' if is_roadmap else '',
                'Alert': f"🚀 [ROADMAP] {alert_type}" if is_roadmap else alert_type,
                'Entry Price (₹)': round(entry, 2),
                'Stop Loss (₹)': round(sl, 2),
                'Target Price (₹)': round(target, 2),
                'Day Change (%)': round(float(last_row['Pct_Change']), 2),
                'RSI': round(rsi_val, 2),
...
def filter_ideal_breakout_stock(df):
    if df.empty:
        return pd.DataFrame()
    cond_alert = df['Alert'].str.contains('⭐|Ultimate|ROADMAP', na=False, regex=True)
...
# --- MERGED TABLE ROW HIGHLIGHTING LOGIC ---
def highlight_buying(row):
    is_roadmap = str(row.get('Roadmap Zone', '')).strip() == '⭐'
    alert = str(row.get('Alert', ''))
    
    # Roadmap (⭐) Stocks - Special Dark Blue Highlight
    if is_roadmap:
        return ['background-color: #0d47a1; color: #ffffff; font-weight: bold'] * len(row)
...
# --- DATA DOWNLOADER WITH PERCENTAGE TRACKING ---
...
def run_streamlit_app():
    ...
    st.caption(
        'Engine Upgraded ⚙️ (Roadmap 50 EMA Strategy, NIFTY 50 Filter & Execution Rank Integrated ⚡)'
    )
