...
def send_email_alert(symbol, entry, sl, target, score, rank, window, condition, max_safe_open):
    ...
    try:
        ...
        <body style="font-family: Arial, sans-serif; background-color: #0d1117; color: #c9d1d9; padding: 20px;">
            <div style="max-width: 520px; background-color: #161b22; padding: 20px; border-radius: 10px; border: 2px solid #28a745; margin: 0 auto;">
                ...
                <p><b>🛡️ Max Safe Open Price:</b> <span style="color: #00ff7f;">₹{max_safe_open} (Max +3% Gap-Up)</span></p>
                <p><b>🛑 Stop Loss:</b> ₹{sl}</p>
                <p><b>🏁 Target Price:</b> ₹{target}</p>
                <hr style="border: 0.5px solid #30363d;">
                <p style="font-size: 12px; color: #ffa500;"><b>⚠️ Safety Warning:</b> If opening gap is greater than ₹{max_safe_open}, DO NOT BUY immediately. Wait for 15-min candle confirmation.</p>
                <p style="font-size: 11px; color: #8b949e;">Sent automatically from Aashiyana Engine 🚀</p>
            </div>
        </body>
        </html>
        """
...
def fetch_mega_nse_universe():
    fallback = [
        ...
        'UPL.NS', 'ULTRACEMCO.NS', 'WIPRO.NS', 'PERSISTENT.NS', 'HAL.NS', 'NATIONALUM.NS', 'BHEL.NS', 'DIXON.NS'
    ]
    try:
...
def analyze_single_ticker(
    ...
    try:
        ...
        if is_signal and pd.notna(last_close_val):
            ...
            # --- NEXT-DAY SAFETY GAP CALCULATION ---
            max_safe_open = round(entry * 1.03, 2)  # Max +3.0% Gap-Up limit
            ...
            if close_pos >= 90.0 and buying_surge_pct >= 200.0:
                ...
                exec_condition = f'Hold above ₹{round(entry, 2)} (Open < ₹{max_safe_open})'
            elif close_pos >= 85.0 and buying_surge_pct >= 150.0:
                exec_rank = '🥈 Rank 2 (High Priority)'
                entry_window = '9:20 AM - 9:35 AM'
                exec_condition = f'Break & Hold above ₹{round(entry, 2)} (Open < ₹{max_safe_open})'
            else:
...
# ==============================================================================
# 3-MONTH HISTORICAL BACKTEST ENGINE (3:30 PM SIGNAL VALIDATION)
# ==============================================================================
def run_historical_backtest(cached_master):
    results = []
    ...
    for ticker, df in cached_master.items():
        ...
        cond_breakout = df['Close'] > df['High_20_Prev']
        cond_vol = df['Volume'] > (df['Vol_SMA20'] * 2.2)
        cond_rsi = (df['RSI'] >= 58) & (df['RSI'] <= 75)
        cond_pct = (df['Pct_Change'] >= 1.0) & (df['Pct_Change'] <= 12.0)

        df['Signal'] = cond_breakout & cond_vol & cond_rsi & cond_pct
        signal_indices = df.index[df['Signal']].tolist()

        for idx in signal_indices:
            loc = df.index.get_loc(idx)
            if loc + 1 < len(df):  # Next day exists
                signal_day = df.iloc[loc]
                next_day = df.iloc[loc + 1]

                trigger = round(float(signal_day['Close']), 2)
                next_open = round(float(next_day['Open']), 2)
                next_high = round(float(next_day['High']), 2)

                gap_pct = round(((next_open - trigger) / trigger) * 100, 2)
                max_gain = round(((next_high - trigger) / trigger) * 100, 2)

                # Safe Rules: Open gap <= 3.0% and Next Day High Gain >= 3.5%
                is_safe = "✅ SAFE BREAKOUT" if (gap_pct <= 3.0 and max_gain >= 3.5) else ("❌ TRAP (Over-gap)" if gap_pct > 3.0 else "🔴 WEAK")
...
# ==============================================================================
# DOWNLOADER & MARKET DATA MANAGEMENT
# ==============================================================================
    ...
    for _, row in alert_candidates.iterrows():
        ...
        if symbol not in already_sent:
            ok = send_email_alert(
                ...
                max_safe_open=row['Max Safe Open (₹)']
            )
            if ok:
...
def run_streamlit_app():
    ...
    st.caption('Engine Upgraded ⚙️ (NIFTY Filter, Next-Day Safety Rules & 3-Month Backtest Integrated ⚡)')

    nifty_info = cached_nifty_status()
    if nifty_info['is_bullish']:
        st.success(
            f"### 🟢 NIFTY 50 TREND STATUS: **{nifty_info['status']}**\n\n"
            f"**Nifty 50 Close:** ₹{nifty_info['nifty_close']} | **20 EMA:** ₹{nifty_info['nifty_ema20']} | **Strength:** +{nifty_info['pct_diff']}% above EMA.\n\n"
            f"🛡️ **Support (S1):** ₹{nifty_info['s1']} | 🎯 **Resistance (R1):** ₹{nifty_info['r1']}"
        )
    else:
        st.error(
            f"### 🔴 NIFTY 50 TREND STATUS: **{nifty_info['status']}**\n\n"
            f"**Nifty 50 Close:** ₹{nifty_info['nifty_close']} | **20 EMA:** ₹{nifty_info['nifty_ema20']} | **Weakness:** {nifty_info['pct_diff']}% below EMA.\n\n"
            f"🛡️ **Support (S1):** ₹{nifty_info['s1']} | 🎯 **Resistance (R1):** ₹{nifty_info['r1']}"
        )
    ...
    tab1, tab2 = st.tabs(["⚡ Live Breakout Scanner", "🧪 3-Month Historical Backtest"])
                ...
                with st.spinner('Searching for breakout setups...'):
                    ...
                    with ThreadPoolExecutor(max_workers=8) as executor:
                        futures = {
                            ...
                            ): ticker for ticker, df in pool.items()
                        }
                        for future in as_completed(futures):
                            res = future.result()
                            if res:
            ...
            if not res_df.empty:
                ...
                if not ideal_matches_df.empty:
                    ...
                    for idx, row in ideal_matches_df.iterrows():
                        ...
                        <p style="color: #00ff7f; font-weight: bold; margin: 0; font-size: 15px;">🎯 Trigger: ₹{p_entry} | 🛡️ Max Safe Open: ₹{max_safe} (+3% Max) | SL: ₹{p_sl} | Target: ₹{p_tgt}</p>
                        </div>"""
    ...
    with tab2:
        st.subheader("📊 3-Month EOD Breakout Performance Analysis")
        st.write("Yeh engine pichle 90 dino me 3:30 PM par aaye saare signals ka **Next-Day Open %** aur **Next-Day High %** check karke Win Rate / Trap Rate batata hai.")

        if 'master_market_data' not in st.session_state:
            st.warning("⚠️ Load data first from sidebar to run backtest.")
        else:
            if st.button("🧪 Run 3-Month Backtest", key="bt_btn"):
                with st.spinner("Analyzing past 90 days breakout history..."):
                    bt_df = run_historical_backtest(st.session_state['master_market_data'])

                if not bt_df.empty:
                    total_trades = len(bt_df)
                    safe_trades = len(bt_df[bt_df['Verdict'].str.contains('SAFE')])
                    traps = len(bt_df[bt_df['Verdict'].str.contains('TRAP')])
                    win_rate = round((safe_trades / total_trades) * 100, 1)

                    col1, col2, col3, col4 = st.columns(4)
                    col1.metric("Total Signals (3 Mo)", total_trades)
                    col2.metric("Safe Breakouts", safe_trades)
                    col3.metric("Over-Gap Traps (>3%)", traps)
                    col4.metric("Success Rate (%)", f"{win_rate}%")

                    st.dataframe(bt_df, hide_index=True)
                else:
                    st.info("No historical breakout signals found in the selected timeframe.")
