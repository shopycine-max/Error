...
def get_mega_nse_universe():
    try:
        ...
        st.sidebar.error("❌ EQUITY_L.csv फ़ाइल नहीं मिली! Defaults use ho rahe hain.")
...
def analyze_single_ticker(ticker, df, mode, volume_multiplier, rsi_filter, turnover_limit, formula_version):
    try:
        ...
        if mode == "live" and df['Signal'].iloc[-1]:
            ...
            # Individual Checklist Criteria Calculation for Quick Filtering
            c_alert = 1 if ('⭐' in alert_type or 'Ultimate' in alert_type) else 0
            c_cont = 1 if close_pos > 85 else 0
            c_surge = 1 if buying_surge_pct > 150 else 0
            c_vol = 1 if vol_spike > 2.5 else 0
            c_accum = 1 if accum_ratio > 1.8 else 0
            c_rsi = 1 if (df['RSI'].iloc[-1] >= 60 and df['RSI'].iloc[-1] <= 72) else 0
            passed_checks = c_alert + c_cont + c_surge + c_vol + c_accum + c_rsi
            ...
            for idx in triggers.index:
                ...
                for f_date, f_row in post_df.iterrows():
                    ...
                    if hit_sl and hit_tgt or hit_sl:
                        outcome = "Hit SL 🛑"
...
def download_all_market_data(tickers):
    ...
    for c_idx, chunk in enumerate(ticker_chunks):
        status_text.text(f"⏳ Fetching Batch {c_idx+1}/{len(ticker_chunks)} ({len(cached_master)} loaded)...")
        try:
...
with tab1:
    ...
    if 'master_market_data' not in st.session_state:
        ...
        if not res_df.empty:
            ...
            # Perfect 6/6 Matches
            ideal_matches_df = res_df[res_df['Passed Checks'] == 6].copy()
            # Near Miss Watchlist (Passed 4 or 5 criteria)
            watchlist_df = res_df[(res_df['Passed Checks'] >= 4) & (res_df['Passed Checks'] < 6)].copy()
            ...
            if not ideal_matches_df.empty:
                ...
                for idx, row in ideal_matches_df.iterrows():
                    rank = idx + 1
                    box_html += f'<div style="border-bottom: 1px dashed #30363d; padding-bottom: 12px; margin-bottom: 12px;"><h3 style="color: #58a6ff; margin: 0;">#{rank} Stock: <u>{row["Symbol"]}</u> (Score: {row["Score"]})</h3><p style="color: #c9d1d9; font-size: 14px; margin-top: 6px; margin-bottom: 6px;"><b>Alert:</b> {row["Alert"]} | <b>Continuation:</b> {row["Continuation Score (%)"]}% | <b>Surge:</b> {row["Massive Buying Surge (%)"]}% | <b>RSI:</b> {row["RSI"]}</p><p style="color: #00ff7f; font-weight: bold; margin: 0; font-size: 15px;">🎯 Trigger: ₹{row["Entry Price (₹)"]} | SL: ₹{row["Stop Loss (₹)"]} | Target: ₹{row["Target Price (₹)"]}</p></div>'
                box_html += '</div>'
                st.markdown(box_html, unsafe_allow_html=True)
                
                # Chart for #1 Stock
                top_stock = ideal_matches_df.iloc[0]['Symbol']
                top_row = ideal_matches_df.iloc[0]
            else:
                st.markdown('<div style="background-color: #161b22; border: 2px solid #ff4d4d; border-radius: 12px; padding: 18px; margin-bottom: 15px;"><h2 style="color: #ff4d4d; margin: 0;">❌ No Perfect 10/10 Breakout Stock Found Today</h2><p style="color: #c9d1d9; font-size: 14px; margin-top: 6px;">आज सभी 6 शर्तों पर 100% खरा उतरने वाला कोई स्टॉक नहीं मिला hai.</p></div>', unsafe_allow_html=True)
                
                if not watchlist_df.empty:
                    st.info(f"👀 **Near-Miss Watchlist ({len(watchlist_df)} Stocks):** Ye stocks 4 ya 5 criteria match kar rahe hain:")
                    st.dataframe(watchlist_df[['Symbol', 'Passed Checks', 'Alert', 'Entry Price (₹)', 'RSI', 'Vol Spike (x)']], use_container_width=True)

            # Enhanced Subplot Chart (Candlestick + Volume + RSI)
            target_chart_stock = ideal_matches_df.iloc[0]['Symbol'] if not ideal_matches_df.empty else (watchlist_df.iloc[0]['Symbol'] if not watchlist_df.empty else res_df.iloc[0]['Symbol'])
            
            st.markdown(f"### 👑 Multi-Pane Technical Chart View: **{target_chart_stock}**")
            chart_data = yf.download(f"{target_chart_stock}.NS", period="3mo", interval="1d", progress=False)
            ...
            if not chart_data.empty:
                ...
                if not chart_data.empty:
                    ...
                    fig.add_trace(go.Scatter(x=chart_data.index, y=chart_data['Close'].ewm(span=20).mean(), line=dict(color='orange', width=1.5), name='EMA 20'), row=1, col=1)
                    
                    # Row 2: Volume Bars
                    colors = ['#00cc66' if c >= o else '#ff4d4d' for c, o in zip(chart_data['Close'], chart_data['Open'])]
                    fig.add_trace(go.Bar(x=chart_data.index, y=chart_data['Volume'], marker_color=colors, name='Volume'), row=2, col=1)
                    
                    # Row 3: RSI Line
                    fig.add_trace(go.Scatter(x=chart_data.index, y=chart_data['RSI'], line=dict(color='#58a6ff', width=1.5), name='RSI'), row=3, col=1)
                    fig.add_hline(y=70, line_dash="dash", line_color="red", row=3, col=1)
                    fig.add_hline(y=60, line_dash="dash", line_color="green", row=3, col=1)

                    fig.update_layout(template="plotly_dark", height=650, xaxis_rangeslider_visible=False, paper_bgcolor='#0d1117', plot_bgcolor='#161b22')
