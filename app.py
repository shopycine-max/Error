...
def fetch_nifty_market_status():
    ...
    return {
        'status': '⚠️ UNKNOWN (Bypassed Rate Limit)', 'is_bullish': True,
        'nifty_close': 0.0, 'nifty_ema20': 0.0, 'pct_diff': 0.0,
        's1': 0.0, 'r1': 0.0, 'sup_20d': 0.0, 'res_20d': 0.0,
    }
...
# ==============================================================================
# STRATEGY INDICATORS GENERATOR (USED FOR BOTH LIVE SCAN & BACKTEST)
# ==============================================================================
def apply_strategy_indicators(df, volume_multiplier=2.2, rsi_filter=58, turnover_limit=3, formula_version='Version 2'):
    df = df.copy()
    ...
    cond_vol_50 = df['Volume'] > df['Vol_SMA50']
    ...
    if 'Version 1' in formula_version or formula_version == 'v1':
        ...
        df['Signal'] = (cond1 & cond2 & cond3 & cond_vol_50 & cond4 & cond5 & 
                        cond7 & cond8 & cond9 & cond10 & cond12 & cond_accum & 
                        cond_no_wick & cond_breakout)
    else:
        df['Signal'] = (cond1 & cond2 & cond3 & cond_vol_50 & cond4 & cond5 & 
...
def analyze_single_ticker(ticker, df, volume_multiplier=2.2, rsi_filter=58, turnover_limit=3, formula_version='Version 2'):
    try:
        df_ind = apply_strategy_indicators(df, volume_multiplier, rsi_filter, turnover_limit, formula_version)
        if df_ind is None or df_ind.empty:
            return None

        is_signal = bool(df_ind['Signal'].values[-1])
        last_close_val = df_ind['Close'].values[-1]

        if is_signal and pd.notna(last_close_val):
            entry = float(last_close_val)
            sl = float(df_ind['Low_5d'].values[-1]) if pd.notna(df_ind['Low_5d'].values[-1]) else entry * 0.95
            if sl >= entry or (entry - sl) / entry < 0.005:
                sl = entry * 0.965
            risk = entry - sl
            target = entry + (2 * risk)

            curr_vol = float(df_ind['Volume'].values[-1])
            avg_vol = float(df_ind['Vol_SMA20'].values[-1])
            vol_spike = curr_vol / avg_vol if avg_vol > 0 else 0
            buying_surge_pct = ((curr_vol - avg_vol) / (avg_vol + 1e-10)) * 100
            accum_ratio = float(df_ind['Accum_Ratio_10d'].values[-1]) if pd.notna(df_ind['Accum_Ratio_10d'].values[-1]) else 1.0

            day_high = float(df_ind['High'].values[-1])
            day_low = float(df_ind['Low'].values[-1])
            ...
            rsi_val = float(df_ind['RSI'].values[-1]) if pd.notna(df_ind['RSI'].values[-1]) else 50.0
            ...
            return [{
                ...
                'Day Change (%)': round(float(df_ind['Pct_Change'].values[-1]), 2),
                'RSI': round(rsi_val, 2),
...
# ==============================================================================
# 1-MONTH BACKTESTING ENGINE
# ==============================================================================
def run_backtest_for_ticker(ticker, df, volume_multiplier, rsi_filter, turnover_limit, formula_version, past_days=22):
    try:
        df_ind = apply_strategy_indicators(df, volume_multiplier, rsi_filter, turnover_limit, formula_version)
        if df_ind is None or df_ind.empty:
            return []
        ...
        trades = []
        recent_df = df_ind.iloc[-past_days:]
        signals = recent_df[recent_df['Signal']]

        for s_date, row in signals.iterrows():
            idx = df_ind.index.get_loc(s_date)
            entry = row['Close']
            sl = row['Low_5d'] if pd.notna(row['Low_5d']) else entry * 0.95
            
            # SL logic adjustments
            if sl >= entry or (entry - sl) / entry < 0.005:
                sl = entry * 0.965
            ...
            # Forward track to check PnL
            if idx + 1 < len(df_ind):
                future_df = df_ind.iloc[idx+1:]
                for f_date, f_row in future_df.iterrows():
                    # Check worst case scenario first (SL hit before Target)
                    if f_row['Low'] <= sl and f_row['High'] >= target:
                        status = 'LOSS' # Assumed loss in whipsaw/gap for conservative backtest
                        exit_date = f_date
                        exit_price = sl
                        break
            ...
            trades.append({
                'Symbol': ticker.replace('.NS', ''),
                'Entry Date': s_date.strftime('%Y-%m-%d'),
                'Entry Price': round(entry, 2),
                'Stop Loss': round(sl, 2),
...
def download_market_data_safe(tickers, period='6mo', interval='1d', chunk_size=40, sleep_sec=0.5, progress_bar=None, status_text=None):
    # Changed period to '6mo' to allow enough data for 50 EMA and a full 22-day backtest.
    cached_master = {}
        ...
        for attempt in range(3):
            try:
                raw_data = yf.download(
                    ...
                    progress=False, group_by='ticker', threads=True, timeout=15, session=session,
                )
                if raw_data.empty:
                    break
...
def run_headless_scan():
    ...
    cached_master = download_market_data_safe(
        tickers, period='6mo', interval='1d', chunk_size=40, sleep_sec=0.5
    )
    ...
    for _, row in alert_candidates.iterrows():
        ...
        if symbol not in already_sent:
            ok = send_email_alert(
                symbol=symbol, entry=row['Entry Price (₹)'], sl=row['Stop Loss (₹)'], target=row['Target Price (₹)'],
...
def run_streamlit_app():
    ...
    st.caption('Engine Upgraded ⚙️ (NIFTY 50 Trend Filter, Execution Rank & Backtesting ⚡)')
    ...
    def run_backtester():
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = {
                executor.submit(run_backtest_for_ticker, ticker, df, volume_multiplier, rsi_filter, min_turnover, formula_version, 22): ticker
                for ticker, df in pool.items()
            }
            for future in as_completed(futures):
                res = future.result()
...
<p style="color: #00ff7f; font-weight: bold; margin: 0; font-size: 15px;">🎯 Trigger: ₹{row['Entry Price (₹)']} | SL: ₹{row['Stop Loss (₹)']} | Target: ₹{row['Target Price (₹)']}</p>
                ...
                # Show Chart for Top Ideal Match
                top_stock = ideal_matches_df.iloc[0]['Symbol']
                ...
                if not chart_data.empty:
                    ...
                    if not chart_data.empty:
                        ...
                        live_sl = ideal_matches_df.iloc[0]['Stop Loss (₹)']
                        live_tgt = ideal_matches_df.iloc[0]['Target Price (₹)']
            ...
            st.caption("No breakout setups currently active. Click 'Run Scanner' above to check.")


        st.markdown("---")
        
        # BACKTESTER UI
        st.subheader('🕰️ 1-Month Strategy Backtest (Last 22 Trading Days)')
        
        if st.button('📈 Run 1-Month Backtest', key='bt_btn'):
            with st.spinner('Simulating historical trades for the past month...'):
                st.session_state['backtest_results'] = run_backtester()

        bt_df = st.session_state.get('backtest_results', pd.DataFrame())

        if not bt_df.empty:
            # Sort by entry date descending
            bt_df = bt_df.sort_values(by='Entry Date', ascending=False)
            
            total_trades = len(bt_df)
            wins = len(bt_df[bt_df['Status'] == 'WIN'])
            ...
            def highlight_status(row):
                ...
                if 'WIN' in status:
                    return ['background-color: rgba(0, 255, 127, 0.2); color: #00ff7f;'] * len(row)
                elif 'LOSS' in status:
                    return ['background-color: rgba(255, 77, 77, 0.2); color: #ff4d4d;'] * len(row)
                elif 'OPEN' in status:
                    return ['background-color: rgba(255, 215, 0, 0.2); color: #ffd700;'] * len(row)
                return [''] * len(row)

            st.write("**Backtested Historical Trades History:**")
            st.dataframe(bt_df.style.apply(highlight_status, axis=1), hide_index=True, use_container_width=True)
        else:
             st.caption("Click 'Run 1-Month Backtest' to see past performance.")
