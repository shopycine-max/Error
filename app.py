...
def send_email_alert(symbol, entry, sl, target, score, rank, window, condition):
    if not SENDER_PASSWORD or not SENDER_EMAIL:
        log_msg('⚠️ Email Credentials Missing (SENDER_EMAIL / SENDER_PASSWORD).', 'warning')
        return False
...
def analyze_single_ticker(
    ...
    rsi_filter=58,
    turnover_limit=5,  # Upper default to 5 Cr to eliminate illiquid traps
    ...
    try:
        ...
        # --- REVISED & STRICT FILTERS TO PREVENT BLOW-OFF EXHAUSTION TRAPS ---
        cond_no_wick = df['Wick_Ratio'] <= 0.15  # Strict wick check (Max 15%)
        cond_breakout = df['Close'] > df['High_20_Prev']
        cond1 = df['Close'] >= 20

        # Prevent large single-day green move (>5.0%) which causes immediate profit booking next day
        cond2 = (df['Pct_Change'] >= 1.0) & (df['Pct_Change'] <= 5.0)

        # Volume Spike strictly between 2.2x and 6.0x (Eliminates Exhaustion volume traps like 8x-13x)
        vol_spike_calc = df['Volume'] / (df['Vol_SMA20'] + 1e-10)
        cond3 = (vol_spike_calc >= volume_multiplier) & (vol_spike_calc <= 6.0)

        cond4 = df['Return_20d'] >= 2.0
        cond5 = df['Turnover'] > (turnover_limit * 10000000)  # Min Turnover ₹5 Crores

        # RSI Filter: Keep RSI between 58 and 68 (Prevents Overbought Exhaustion above 68)
        cond8 = (df['RSI'] >= rsi_filter) & (df['RSI'] <= 68.0)

        # Price must stay within 5% of 20 EMA to prevent mean reversion trap
        cond9 = (df['Close'] > df['EMA_20']) & (df['Close'] <= df['EMA_20'] * 1.05)

        # Accumulation Ratio strictly bounded between 1.5 and 5.0 (Cuts hyper-extended runners)
        cond_accum = (df['Accum_Ratio_10d'] >= 1.5) & (df['Accum_Ratio_10d'] <= 5.0)

        # Trend alignment requirement
        cond_trend = df['EMA_20'] > df['EMA_50']
        ...
        if is_signal and pd.notna(last_close_val):
            ...
            # Cap buying surge to eliminate exhaustion spikes (>450%)
            if buying_surge_pct > 450.0:
                return None
            ...
            if close_pos < 88.0:
                return None  # Rejects candles with upper wicks

            if close_pos >= 92.0 and buying_surge_pct >= 200.0:
                exec_rank = '🥇 Rank 1 (Top Winner)'
                entry_window = '9:15 AM - 9:30 AM'
                exec_condition = f'Hold above ₹{round(entry, 2)}'
            elif close_pos >= 88.0 and buying_surge_pct >= 120.0:
            ...
            alert_type = '⭐ Quality Breakout Setup'

            rsi_val = (
                float(df['RSI'].values[-1]) if pd.notna(df['RSI'].values[-1]) else 50.0
            )

            # Capped Score Model: Prevents runaway values from overriding healthy technical setups
            capped_vol = min(vol_spike, 5.0)
            capped_accum = min(accum_ratio, 4.0)
            total_score = round(
                rsi_val
                + (capped_vol * 10)
                + (capped_accum * 12)
...
def filter_ideal_breakout_stock(df):
    ...
    cond_surge = (df['Massive Buying Surge (%)'] >= 100.0) & (df['Massive Buying Surge (%)'] <= 450.0)
    cond_vol = (df['Vol Spike (x)'] >= 2.2) & (df['Vol Spike (x)'] <= 6.0)
    cond_accum = (df['Accum Ratio (10d)'] >= 1.5) & (df['Accum Ratio (10d)'] <= 5.0)
    cond_rsi = (df['RSI'] >= 58.0) & (df['RSI'] <= 68.0)
...
def run_3month_backtest(master_data, backtest_days=60):
    ...
    for ticker, df in master_data.items():
        ...
        cond2 = (df_calc['Pct_Change'] >= 1.0) & (df_calc['Pct_Change'] <= 5.0)

        vol_spike_calc = df_calc['Volume'] / (df_calc['Vol_SMA20'] + 1e-10)
        cond3 = (vol_spike_calc >= 2.2) & (vol_spike_calc <= 6.0)

        cond4 = df_calc['Return_20d'] >= 2.0
        cond5 = df_calc['Turnover'] > (5 * 10000000)
        cond8 = (df_calc['RSI'] >= 58.0) & (df_calc['RSI'] <= 68.0)
        cond9 = (df_calc['Close'] > df_calc['EMA_20']) & (df_calc['Close'] <= df_calc['EMA_20'] * 1.05)
        cond_accum = (df_calc['Accum_Ratio_10d'] >= 1.5) & (df_calc['Accum_Ratio_10d'] <= 5.0)
        cond_trend = df_calc['EMA_20'] > df_calc['EMA_50']
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(futures):
            ...
            msg = (
                ...
                f' ({min(completed_tickers, total_tickers)}/{total_tickers} stocks)'
            )
            if IS_HEADLESS:
                log_msg(msg, 'info')
            else:
...
def run_streamlit_app():
    ...
    st.caption(
        'Engine Upgraded ⚙️ (Exhaustion Trap Filter & Capped Score Roadmap Enabled ⚡)'
    )
    ...
    if nifty_info['is_bullish']:
        st.success(
            ...
            f"**Nifty 50 Close:** ₹{nifty_info['nifty_close']} | **20 EMA:**"
            f" ₹{nifty_info['nifty_ema20']} | **Strength:** +{nifty_info['pct_diff']}%"
            ' above EMA. **(Take Fresh Long Trades)**\n\n'
            f"🛡️ **Immediate Support (S1):** ₹{nifty_info['s1']} | **20-Day"
            f" Support (Low):** ₹{nifty_info['sup_20d']}\n\n"
            f"🎯 **Immediate Resistance (R1):** ₹{nifty_info['r1']} | **20-Day"
            f" Resistance (High):** ₹{nifty_info['res_20d']}"
        )
    else:
        st.error(
            f"### 🔴 NIFTY 50 TREND STATUS: **{nifty_info['status']}**\n\n"
            f"**Nifty 50 Close:** ₹{nifty_info['nifty_close']} | **20 EMA:**"
            f" ₹{nifty_info['nifty_ema20']} | **Weakness:** {nifty_info['pct_diff']}%"
            ' below EMA. **(Avoid New Long Positions)**\n\n'
            f"🛡️ **Immediate Support (S1):** ₹{nifty_info['s1']} | **20-Day"
            f" Support (Low):** ₹{nifty_info['sup_20d']}\n\n"
            f"🎯 **Immediate Resistance (R1):** ₹{nifty_info['r1']} | **20-Day"
            f" Resistance (High):** ₹{nifty_info['res_20d']}"
...
<p style="color: #00ff7f; font-weight: bold; margin: 0; font-size: 15px;">🎯 Trigger: ₹{p_entry} | SL: ₹{p_sl} | Target: ₹{p_tgt}</p>
                ...
                st.caption(
                    "No breakout setups currently active. Click 'Run Scanner' above."
                )
    ...
    with tab2:
        ...
        if 'master_market_data' not in st.session_state:
            ...
            if not bt_history.empty:
                ...
                st.caption(
                    'Backtest run karne ke liye uper button par click kijiye.'
                )
